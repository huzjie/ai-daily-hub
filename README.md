...
|---|---|---|---|---|---|
| 2026-09-26 | 差分注意力（Differential Transformer）——每个头算两个 softmax 注意力图再相减，抵消公共噪声成分，降低幻觉、提升长上下文检索，几乎不增加参数量、KV cache 还可减半 | [**diffforge（差分注意力 Transformer 训练与推理框架）**](https://github.com/huzjie/diffforge) | Python 3.9+ 零依赖内核 + 差分注意力（λ 可学习，层感知初始化 λ_init=0.8-0.6·exp(-0.3(l-1))，头级 GroupNorm）+ 参考模型（Embedding+预归一化 Block+RoPE+YaRN+SwiGLU+LM head，纯 Python）+ 确定性可训练 Mock 后端（MD5 种子，真理性 0.49→1.00、32K 检索 0.06→0.99，训练即学 λ）+ 四组基准（passkey 大海捞针 / hallucination 事实性 / perplexity / λ 分析）+ stdlib HTTP 服务（Bearer 鉴权 + 客户端）+ CLI（doctor/train/bench/generate/serve/analyze）+ 模型卡 + Docker/K8s/Helm/CI | 166 文件 / 85 Python / 56 MD / 29 单测 | ✅ 已发布 |
| 2026-09-25 | 基元律动开源 NeoHorse-Jev-4B 实时决策模型（4B 毫秒级决策，贪吃蛇 3000 步/89 目标存活，六组开源评测综合 AVG 77.70 第一）——核心是「实时决策 + 前瞻安全」，每一步都要快速选对，还不能把后续空间走死 | [**rhythmforge（实时决策引擎 + 前瞻安全规划框架）**](https://github.com/huzjie/rhythmforge) | Python 3.9+ 零依赖内核 + 毫秒级决策引擎（枚举→打分→softmax→选择，mock 约 0.01ms/次）+ 前瞻安全规划器（Beam Search / Monte-Carlo / Hybrid + 生存空间 flood-fill 评估，贪心 203 步撞墙 vs 规划 800 步存活）+ 可插拔后端（可训练确定性 Mock / OpenAI 兼容 / vLLM / Transformers）+ 八组基准评测（Nimble/VitaminC/MASSIVE/Transfer/Knowledge/Semantic/Reflex/Safety + AVG 综合分，默认 AVG 80）+ 智能体循环 + 情景记忆 + 重放缓冲 + 可观测性（Prometheus 导出）+ 五类决策环境（贪吃蛇/网格世界/井字棋/寻路/多臂老虎机）+ 零依赖 stdlib HTTP 服务（Bearer 鉴权 + 限流 + 客户端）+ CLI（doctor/demo/bench/snake/plan/serve）+ 模型卡 + Docker/K8s/Helm/CI | 151 文件 / 97 Python / 24 MD / 23 单测 | ✅ 已发布 |
| 2026-09-24 | 把开源 LLM 从「会说话的评委」变成**可校准、抗顺序偏差、带置信度门控的类型化决策模型**——Nokia AnyJev 方向：无需额外训练即可把开源模型用于路由分流、风险筛查、工具授权、自动化支持决策，核心是三类失效的系统性治理：概率失准（过度自信）、选项顺序偏差（换顺序换答案）、标签/先验偏差 | [**calibrex（LLM 决策校准与门控引擎）**](https://github.com/huzjie/calibrex) | Python 3.9+ 零依赖内核 + 排列评分（全排列/抽样排列 + 跨排列聚合，消除选项顺序偏差）+ 五类校准器（温度缩放/Platt/保序回归 PAV/直方图分箱/Beta 校准）+ 集成选优（留出集 NLL）+ 去偏差（标签频率校正/先验偏移校正）+ 五类门控（置信度阈值/误判风险预算/熵/margin/多样性）+ 决策流水线（排列→校准→去偏→门控）+ 7 后端（可训练确定性 Mock/OpenAI 兼容/vLLM/Transformers/Anthropic/Azure/Gemini）+ 完整指标（ECE/MCE/Brier/NLL/准确率/AUROC/可靠性曲线/PR-ROC）+ 四类决策处理器（分类/排序/评分/抽取）+ 评测基准套件 + 零依赖 stdlib HTTP 服务（含 Bearer 鉴权/限流 + 客户端）+ CLI（doctor/demo/evaluate/calibrate/predict/serve）+ 10 模型卡 + Docker/K8s/Helm/CI | 192 文件 / 119 Python / 53 MD | ✅ 已发布 |
| 2026-09-23 | 理想汽车 Foundation Model 团队开源 **ME-Dex-1.0 世界动作触觉模型**（把触觉与视频共同当作需预测的未来世界状态，联合去噪视频/触觉/动作序列，三专家架构 + H-Bridge 共享注意力 + 双手 34 区域模板 + 自主式触觉数据引擎） | [**dexworld（多模态触觉世界模型训练与推理框架）**](https://github.com/huzjie/dexworld) | Python 3.8+ 零依赖内核 + 34 区域触觉模板 + 异构传感器映射（GelSight/触觉阵列/力扭矩）+ 三专家编码（视觉/触觉/动作）+ H-Bridge 共享注意力 + 联合去噪扩散世界模型 + 自主式触觉仿真数据引擎 + 数据增强/经验回放/课程学习 + MPC/Beam/CEM 规划 + 集成预测/消融实验/交叉验证 + ASCII 可视化 + Mock（可训练）/OpenAI 兼容后端 + CLI + 零依赖 HTTP 服务 + Docker/K8s/Helm/CI | 170 文件 / 126 Python / 25 MD / 40 单测 | ✅ 已发布 |
| 2026-09-22 | 小米发布并开源 Xiaomi MiMo-V2.6 全模态大模型（Pro 登顶全球开源榜首，AA 智能指数 46 分），核心是探索 **RSI（递归自我改进）** 路径：以可验证复杂任务为基础、规模化扩展 RL 算力、Group 内相对比较奖励信号 | [**rsiforge（RSI 递归自我改进强化学习训练框架）**](https://github.com/huzjie/rsiforge) | Python 3.9+ 零依赖内核 + 4 领域可验证任务环境（code/general/visual/cyber）+ 合成任务生成器 + 难度课程 + 轨迹采集（SQLite/JSON/去重）+ 奖励体系（规则/执行/LLM-as-Judge + Group 相对比较 + 优势缩放）+ 三大 RL 算法（GRPO/PPO/DPO + 优势估计 + KL 控制）+ 四模型后端（可训练 Mock/OpenAI 兼容/vLLM/HF）+ 蒸馏/检查点/断点续训/模型合并 + 基准评测/排行榜 + 零依赖 HTTP 服务/FastAPI + 可观测 + CLI + Docker/K8s/Helm/CI | 180 文件 / 119 Python / 31 MD / 33 单测 | ✅ 已发布 |
| 2026-09-21 | Anthropic 发布 MCP 2.1——引入**实时流式上下文更新**（AI 助手通过增量事件接收上下文变化，无需每次全量重载，长会话上下文加载延迟最高降 75%）+ 开源 AgentKit 1.0 稳定版（工具调用/记忆/错误恢复） | [**streamctx（MCP 2.1 流式上下文引擎）**](https://github.com/huzjie/streamctx) | Python 3.9+ 零依赖内核 + 增量 diff（行级 LCS 最小化差异）+ 版本向量/Lamport 时钟 + Token 预算（纯 Python 估算 + 加权 LRU 驱逐）+ 快照/增量自适应降级 + 断点续传（checkpoint/resume）+ 三种传输（进程内/SSE/stdlib/WebSocket）+ 主题订阅（glob 模式）+ 插件系统 + 可观测（指标/追踪/健康）+ mock/openai/anthropic/ollama 上下文适配器 + stdlib SSE/FastAPI 双控制面 + CLI + Docker/K8s/Helm/CI | 130 文件 / 80 Python / 22 单测 | ✅ 已发布 |
| 2026-09-20 | 清华 × Infinigence 开源 ICLR 2026 论文 C2C（Cache-to-Cache）——多智能体之间不再生成自然语言文本，而是把发送方 KV Cache 经「神经融合器」旋转、嫁接进接收方层里，由可学习门控决定哪些层吸收，约 2.5× 推理加速 / +14.2% 精度 | [**c2cforge（Cache-to-Cache 免文本多智能体通信框架）**](https://github.com/huzjie/c2cforge) | 神经融合器（正交 Procrustes 旋转对齐 + 层嫁接 + 可学习/ TopK / 均匀门控 + rotate_graft/concat/residual/attention 四种融合模式）+ KV Cache 引擎（分层结构/序列化 numpy·safetensors·pickle·msgpack/压缩 zstd·gzip·lz4/增量传输/内容寻址/LRU·LFU·FIFO 驱逐/持久化存储）+ 多智能体运行时（cache/text 双协议 + 路由 + 调度 + pipeline/star/mesh/ring 拓扑 + 团队编排）+ 模型后端（mock 零依赖/transformers/vllm 可插拔）+ FastAPI REST/CLI/SDK + 评测基准（加速比/延迟/token 节省）+ 可观测性（Prometheus/span/健康）+ 插件 + 安全护栏 + Docker/K8s/CI | 137 文件 / 91 Python | ✅ 已发布 |
| 2026-09-19 | Google 开源 Agent Substrate（高密度智能体沙箱运行时，空闲 agent 挂起/500ms 恢复完整状态，单机打包 1000+ 休眠环境、比标准容器密度高 10 倍，跑在 microVM/gVisor，核心是「停靠式调度」+ 亚秒快照恢复） | [**substratforge（高密度智能体基板运行时）**](https://github.com/huzjie/substratforge) | 三档隔离沙箱（process/gVisor/microVM 可插拔）+ 快照引擎（tar + 块级增量 + zstd/gzip/lz4 压缩）+ 密度感知调度（装箱/空闲挂起/冷停靠驱逐 LRU/LFU/FIFO/冷热晋升）+ 快照存储（本地/S3/MinIO/内存 + CAS + GC）+ agent 运行时（生命周期状态机 + 心跳看门狗 + 任务队列）+ 服务层（FastAPI REST/CLI/SDK）+ 可观测性（Prometheus 指标/span 追踪/健康检查）+ 安全（最小隔离准入/配额/护栏/审计）+ 插件系统 + 基准框架 + Docker/K8s/CI | 164 文件 / 106 Python | ✅ 已发布 |
| 2026-09-18 | 清华 × Z.AI 开源 ScaleCUA（可扩展任务合成 + 在线强化学习 GUI Agent 框架，9B 模型刷新开源 SOTA：OSWorld 68.7% / ScienceBoard 54.0%，反超 Kimi K2.5 63.3% 与 Claude Sonnet 4.5 62.9%，核心是 VeriGen 可验证任务合成 + RLVR 可验证奖励在线强化学习） | [**cuforge（可验证任务合成驱动的计算机使用智能体在线强化学习平台）**](https://github.com/huzjie/cuforge) | VeriGen 可验证任务合成（6 任务模板 + 11 可验证判定器）+ 虚拟桌面环境（文件系统/UI 组件树/终端/设置/浏览器，确定性可判定）+ 计算机使用智能体运行时（Scripted/LLM 双策略 + 视觉上下文切分 recent/summary/full）+ RLVR 训练（GRPO 组相对策略优化 + 前沿采样 + 结果/过程/格式三路奖励）+ 多 LLM 后端（mock/openai兼容/vllm/ollama/llamacpp/sglang/tokenspeed/anthropic/gemini）+ 安全（沙箱/动作白名单/护栏/审计）+ 可观测性（tracing/metrics）+ 评测基准（mini/OSWorld/ScienceBoard/Terminal-Bench/WebArena）+ FastAPI REST/CLI/SDK + Docker/K8s/CI | 161 文件 / 110 Python | ✅ 已发布 |
...
**热点背景**：2026-09-11 京东探索研究院在 JDD 大会开源视听世界模型 **JoyAI-EchoWM**——在同一个框架内同时生成视频、环境音、音乐与语音，并实时响应用户操作。核心创新是统一「**相机意图（Camera-Intent）**」表示，把键盘/手柄离散操作映射为连续 **6-DoF 轨迹**，让第一人称与第三人称共用同一套控制接口。在 158 个 WBench Navigation 案例上取得 81.7 平均分（一致性 89.8、交互性 87.2），但尚无显式持久三维记忆，长时生成仍可能漂移。

**项目定位**：不训练模型，而是把 EchoWM 的「统一相机意图 + 同框架视听生成 + 实时交互」范式工程化为一个填配置即运行、可直接部署的视听世界模型平台，无 GPU 无网络也能用确定性内核（程序化场景 + DSP 音频）端到端跑通全链路。

**核心能力**：
- 🎮 **统一相机意图**：键盘/文本/JSON 操作 → 相机意图 → 连续 6-DoF 轨迹（四元数 slerp + Catmull-Rom）
- 🎬 **同框架视听生成**：视频帧（射线求交渲染）+ 环境音（1/f 噪声）+ 音乐（和弦/旋律）+ 语音（共振峰）
- 🔄 **实时交互闭环**：输入 → 更新位姿 → 生成视听 → 输出，第一/第三人称统一
- 🧠 **视听记忆**：关键帧 + 音频指纹，缓解长时生成漂移
- 📏 **WBench 风格评测**：一致性 / 交互性 / 连贯性三项指标 + 18 案例
- 🎧 **完整音频引擎**：环境音 7 预设、音乐 4 风格、混响、空间双耳音频、音效
- 🌆 **程序化场景**：corridor/outdoor/city/ocean 四场景 + 后处理 + PNG 帧编码
